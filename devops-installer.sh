...
enabled=1
gpgcheck=1
OBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBgpgkey=https://rpm.grafana.com/gpg.key
EOF
OBOBOBOB                sudo yum install -y grafana
            else
OBOBOBOBOBOBOBOBOBOBOBOBOB                sudo mkdir -p /etc/apt/keyrings/
OBOBOB                wget -q -O - https://apt.grafana.com/gpg.key | gpg --dearmor | sudo tee /etc/apt/keyrings/grafana.gpg > /dev/null
OBOBOB                echo "deb [signed-by=/etc/apt/keyrings/grafana.gpg] https://apt.grafana.com stable main" | sudo tee /etc/apt/sources.list.d/grafana.list
OBOBOB                sudo apt update && sudo apt install -y grafana
            fi
OBOBOBOBOBOB            ;;
OBOBOBOBOBOBOBOB        loki)
            LOKI_VERSION=$(curl -s https://api.github.com/repos/grafana/loki/releases/latest | grep tag_name | cut -d '"' -f 4 | sed 's/v//')
OBOBOBOBOB            wget https://github.com/grafana/loki/releases/download/v${LOKI_VERSION}/loki-linux-amd64.zip
            unzip loki-linux-amd64.zip
OBOBOBOBOBOBOBOBOBOBOBOB            sudo mv loki-linux-amd64 /usr/local/bin/loki
            rm loki-linux-amd64.zip
OBOBOBOBOBOBOBOBOBOB            ;;
OB        promtail)
            LOKI_VERSION=$(curl -s https://api.github.com/repos/grafana/loki/releases/latest | grep tag_name | cut -d '"' -f 4 | sed 's/v//')
OBOBOBOBOBOBOBOBOBOBOB            wget https://github.com/grafana/loki/releases/download/v${LOKI_VERSION}/promtail-linux-amd64.zip
            unzip promtail-linux-amd64.zip
OBOBOB            sudo mv promtail-linux-amd64 /usr/local/bin/promtail
            rm promtail-linux-amd64.zip
OBOB            ;;
        argocd)
            curl -sSL -o /tmp/argocd https://github.com/argoproj/argo-cd/releases/latest/download/argocd-linux-amd64
OBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOB            sudo install -m 555 /tmp/argocd /usr/local/bin/argocd
OBOBOBOBOBOBOB            rm /tmp/argocd
            ;;
OBOBOBOBOBOBOBOBOBOBOBOBOB        mongodb)
OBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOB            if [ "$OS_FAMILY" = "redhat" ]; then
                echo '[mongodb-org-7.0]' | sudo tee /etc/yum.repos.d/mongodb-org-7.0.repo
OBOBOBOBOBOB                echo 'name=MongoDB Repository' | sudo tee -a /etc/yum.repos.d/mongodb-org-7.0.repo
...
OBOBOBOBOBOB                echo 'gpgcheck=1' | sudo tee -a /etc/yum.repos.d/mongodb-org-7.0.repo
...
OBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOB                sudo yum install -y mongodb-org
            else
OBOBOBOBOB                curl -fsSL https://pgp.mongodb.com/server-7.0.asc | sudo gpg -o /usr/share/keyrings/mongodb-server-7.0.gpg --dearmor
                echo "deb [ arch=amd64,arm64 signed-by=/usr/share/keyrings/mongodb-server-7.0.gpg ] https://repo.mongodb.org/apt/ubuntu jammy/mongodb-org/7.0 multiverse" | sudo tee /etc/apt/sources.list.d/mongodb-org-7.0.list
                sudo apt update && sudo apt install -y mongodb-org
            fi
OBOBOBOBOBOBOBOBOBOBOBOBOBOBOB            ;;
OBOBOBOB        github)
OBOBOBOB            if [ "$OS_FAMILY" = "redhat" ]; then
OBOBOBOB                sudo dnf config-manager --add-repo https://cli.github.com/packages/rpm/gh-cli.repo
                sudo yum install -y gh
OBOBOBOBOBOBOB            else
                curl -fsSL https://cli.github.com/packages/githubcli-archive-keyring.gpg | sudo dd of=/usr/share/keyrings/githubcli-archive-keyring.gpg
                echo "deb [arch=$(dpkg --print-architecture) signed-by=/usr/share/keyrings/githubcli-archive-keyring.gpg] https://cli.github.com/packages stable main" | sudo tee /etc/apt/sources.list.d/github-cli.list
OBOBOBOBOBOBOBOBOBOBOB                sudo apt update && sudo apt install -y gh
            fi
OBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOB            ;;
        git)
OBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOB            if [ "$OS_FAMILY" = "redhat" ]; then
                sudo yum install -y git
OBOBOBOBOB            else
                sudo apt update && sudo apt install -y git
OBOBOBOBOBOBOBOB            fi
OBOBOB            ;;
OBOBOB        java)
OBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOB            if [ "$OS_FAMILY" = "redhat" ]; then
OBOBOB                sudo yum install -y java-11-openjdk
OBOB            else
OBOB                sudo apt update && sudo apt install -y default-jdk
            fi
            ;;
OBOB        python)
OBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOB            if [ "$OS_FAMILY" = "redhat" ]; then
                sudo yum install -y python3 python3-pip
OBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOB            else
                sudo apt update && sudo apt install -y python3 python3-pip
OBOBOBOBOBOBOBOBOBOB            fi
            ;;
OB        nodejs)
            if [ "$OS_FAMILY" = "redhat" ]; then
OBOBOBOBOBOBOBOBOBOBOB                sudo yum install -y nodejs npm
OBOBOB            else
OBOBOB                sudo apt update && sudo apt install -y nodejs npm
            fi
...
OBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOB                sudo apt update && sudo apt install -y nginx
OBOB            fi
            ;;
OBOBOBOBOBOBOBOBOBOB        terraform)
            if [ "$OS_FAMILY" = "redhat" ]; then
OBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOB                sudo yum install -y yum-utils
                sudo yum-config-manager --add-repo https://rpm.releases.hashicorp.com/RHEL/hashicorp.repo
OBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOB                sudo yum install -y terraform
            else
                wget -O- https://apt.releases.hashicorp.com/gpg | gpg --dearmor | sudo tee /usr/share/keyrings/hashicorp-archive-keyring.gpg
                echo "deb [signed-by=/usr/share/keyrings/hashicorp-archive-keyring.gpg] https://apt.releases.hashicorp.com $(lsb_release -cs) main" | sudo tee /etc/apt/sources.list.d/hashicorp.list
                sudo apt update && sudo apt install -y terraform
OBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOB            fi
OBOBOBOBOBOBOBOBOBOB            ;;
OBOBOBOBOBOBOBOBOBOBOBOB        docker)
            if [ "$OS_FAMILY" = "redhat" ]; then
OBOBOBOBOBOBOBOBOBOBOBOBOBOB                sudo yum install -y docker
OBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOB                sudo systemctl start docker
                sudo systemctl enable docker
OBOBOBOB            else
                sudo apt update && sudo apt install -y docker.io
OBOBOBOBOBOBOBOBOBOBOB                sudo systemctl start docker
                sudo systemctl enable docker
OBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOB            fi
            ;;
OBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOB        mysql)
            if [ "$OS_FAMILY" = "redhat" ]; then
                sudo yum install -y mysql-server
OBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOB            else
OBOBOBOB                sudo apt update && sudo apt install -y mysql-server
OBOBOB            fi
            ;;
OBOBOBOBOB        postgresql)
OBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOB            if [ "$OS_FAMILY" = "redhat" ]; then
                sudo yum install -y postgresql postgresql-server
OBOBOBOBOBOBOB            else
                sudo apt update && sudo apt install -y postgresql postgresql-contrib
            fi
OBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOB            ;;
        kubernetes)
OBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOB            if [ "$OS_FAMILY" = "redhat" ]; then
                cat <<EOF | sudo tee /etc/yum.repos.d/kubernetes.repo
OBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOB[kubernetes]
name=Kubernetes
OBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBbaseurl=https://pkgs.k8s.io/core:/stable:/v1.28/rpm/
OBOBenabled=1
OBOBOBgpgcheck=1
OBgpgkey=https://pkgs.k8s.io/core:/stable:/v1.28/rpm/repodata/repomd.xml.key
OBOBEOF
                sudo yum install -y kubectl
OB            else
OBOBOBOB                curl -fsSL https://pkgs.k8s.io/core:/stable:/v1.28/deb/Release.key | sudo gpg --dearmor -o /etc/apt/keyrings/kubernetes-apt-keyring.gpg
OBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOB                echo 'deb [signed-by=/etc/apt/keyrings/kubernetes-apt-keyring.gpg] https://pkgs.k8s.io/core:/stable:/v1.28/deb/ /' | sudo tee /etc/apt/sources.list.d/kubernetes.list
                sudo apt update && sudo apt install -y kubectl
OBOBOBOBOBOBOBOB            fi
            ;;
OBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOB        jenkins)
            if [ "$OS_FAMILY" = "redhat" ]; then
OBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOB                sudo wget -O /etc/yum.repos.d/jenkins.repo https://pkg.jenkins.io/redhat-stable/jenkins.repo
OBOBOBOBOBOBOBOBOBOBOBOBOB                sudo rpm --import https://pkg.jenkins.io/redhat-stable/jenkins.io.key
                sudo yum install -y fontconfig java-17-openjdk jenkins
OBOBOBOBOBOBOBOBOB                sudo systemctl start jenkins
OBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOB                sudo systemctl enable jenkins
            else
                curl -fsSL https://pkg.jenkins.io/debian-stable/jenkins.io-2023.key | sudo tee /usr/share/keyrings/jenkins-keyring.asc > /dev/null
                echo deb [signed-by=/usr/share/keyrings/jenkins-keyring.asc] https://pkg.jenkins.io/debian-stable binary/ | sudo tee /etc/apt/sources.list.d/jenkins.list > /dev/null
OBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOB                sudo apt update && sudo apt install -y fontconfig openjdk-17-jre jenkins
OBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOB                sudo systemctl start jenkins
OBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOBOB                sudo systemctl enable jenkins
            fi
...
OBOBOBOBOBOBOBOBOBOBOBOBOB            fi
            ;;
OBOBOBOBOBOBOBOBOBOBOBOBOBOB        helm)
            curl https://raw.githubusercontent.com/helm/helm/main/scripts/get-helm-3 | bash
OBOBOBOBOB            ;;
        vault)
OBOBOBOBOBOBOB            if [ "$OS_FAMILY" = "redhat" ]; then
                sudo yum-config-manager --add-repo https://rpm.releases.hashicorp.com/RHEL/hashicorp.repo
